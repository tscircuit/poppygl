...
function getBufferViewSlice(
	...
	if (!bufferView) throw new Error(`Invalid bufferView index ${bufferViewIndex}`);
	...
	const viewArray = new Uint8Array(buffer.buffer, buffer.byteOffset + viewOffset, totalByteLength);
	return {
		array: new Uint8Array(viewArray.buffer, baseOffset, totalByteLength - byteOffset),
		stride: bufferView.byteStride ?? 0,
		byteOffsetInView: baseOffset,
	};
}

function readAccessorAsFloat32(ctx: ReadAccessorContext, accessorIndex: number): Float32Array {
	const { gltf } = ctx;
	...
	if (!compInfo || !numComponents)
		throw new Error(`Unsupported accessor componentType ${accessor.componentType}`);
	const byteOffset = accessor.byteOffset ?? 0;
	const { array: src, stride: bufferViewStride, byteOffsetInView } = getBufferViewSlice(
		ctx,
		accessor.bufferView,
		byteOffset,
	);
	...
	const canUseTightView =
		stride === compInfo.size * numComponents && absoluteByteOffset % compInfo.size === 0;
	if (canUseTightView) {
		const TypedArrayCtor = compInfo.array as any;
		const typed = new TypedArrayCtor(src.buffer, src.byteOffset, count * numComponents);
...
function readIndices(ctx: ReadAccessorContext, accessorIndex: number): Uint32Array {
	const { gltf } = ctx;
	const accessor = gltf.accessors?.[accessorIndex];
	if (!accessor) throw new Error(`Invalid accessor index ${accessorIndex}`);
	if (accessor.type !== "SCALAR") throw new Error("Index accessor must be SCALAR");
	const compInfo = COMPONENT_INFO[accessor.componentType];
	if (!compInfo)
...
export function createSceneFromGLTF(gltf: GLTF, resources: GLTFResources): GLTFScene {
		...
		if (Number.isInteger(node.mesh)) {
			...
			if (mesh) {
				for (const primitive of mesh.primitives || []) {
					...
					const material = getMaterial(gltf, textures, resources.images, primitive.material);
					drawCalls.push({
						positions,
						normals,
						uvs,
