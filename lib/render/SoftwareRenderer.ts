...
export class SoftwareRenderer {
	...
	drawMesh(
		...
		for (let i = 0; i < idx.length; i += 3) {
			...
			if (!(isFinite(vInvW[i0]) && isFinite(vInvW[i1]) && isFinite(vInvW[i2]))) continue;
			...
			for (let y = minY; y <= maxY; y++) {
				for (let x = minX; x <= maxX; x++) {
					...
					const ndotl = Math.max(0, nrm[0] * -L[0] + nrm[1] * -L[1] + nrm[2] * -L[2]);
...
function edge(a: readonly [number, number], b: readonly [number, number], p: readonly [number, number]) {
